...
Storage backend: Garage S3 with **three distinct buckets** -- Mimir
...
4. **As a blocking final task**, polls Mimir readiness via
   ...
   `mimir-ruler`, `mimir-alerts`) exist via `docker_container_exec`
   against the running Garage container.
...
The Phase 8 garage role bootstrap creates all three buckets; this role
references them by name. Renaming any bucket here MUST be paired with
renaming in `roles/garage/defaults/main.yml` or the
`telemetron_garage_buckets` list in `storage.yml`.
...
| `mimir_s3_endpoint` | `garage:3900` | **NO** http:// prefix (Mimir/Tempo format); insecure flag signals HTTP |
...
| Key | Purpose |
|-----|---------|
| `mimir_s3_access_key` | Garage S3 access key for all three mimir-* buckets (auto-generated by bootstrap) |
| `mimir_s3_secret_key` | Garage S3 secret key for all three mimir-* buckets (auto-generated by bootstrap) |
...
Future hardening milestone splits Garage into per-backend keys (secret
...
- **Inter-component traffic on the `telemetron` bridge only.** Mimir
  reaches Garage at `garage:3900` over the bridge; never via the host.
...
- `mimir_config == 'standalone'` conditional -- dropped; M1 ships
  monolithic only.
- `mimir_storage_backend: filesystem` -- replaced with `s3` (Garage)
  per D-39.
...
`telemetron_garage_buckets` list entries in `storage.yml`. All three
