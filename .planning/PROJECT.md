...
---
*Last updated: 2026-05-19 after Phase 5 plans 05-05/06/07/08 (full gap-closure cycle) — UI plane complete: Grafana OSS 13.0.1 with 4-datasource provisioning at hardcoded UIDs + 7 curated dashboards + tracesToLogsV2/derivedFields trace-to-logs (UI-04), Karma v0.130 on Alertmanager via Docker bridge DNS (UI-05), PromLens v0.3.0 marked deprecation candidate (UI-06); `playbooks/deploy_docker.yml` reaches `… → alertmanager → grafana → karma → promlens`; 12/13 roles ported (1 remaining: `nfsd`, opt-in for Phase 6); six live-UAT issues from the leviathan round resolved across 05-05/06/07/08: grafana verify.yml `docker_container_exec` rewrite (auto_remove race), `_rewrite_uids.py` target-level walker + 4 upstream UIDs normalized in tempo-self-metrics, Loki `structured_metadata` + regex-fallback `derivedFields`, Karma scratch-image healthcheck flipped opt-in + 5 latent auto_remove sites dropped in karma/promlens verify; re-verification shows 12/12 static must-haves PASS with `gaps_remaining: []` and `regressions: []`; 3 items pending final live-stack UAT on leviathan (`.planning/phases/05-ui-plane/05-HUMAN-UAT.md`) — combined-deploy regression canary, UI-04 click-through with operator-supplied OTel traffic, and tempo-self-metrics 74-panel runtime render*
