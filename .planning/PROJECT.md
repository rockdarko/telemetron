...
---
*Last updated: 2026-05-19 after Phase 5 plan 05-04 (gap closure) — UI plane complete: Grafana OSS 13.0.1 with 4-datasource provisioning at hardcoded UIDs + 7 curated dashboards + tracesToLogsV2/derivedFields trace-to-logs (UI-04), Karma v0.130 on Alertmanager via Docker bridge DNS (UI-05), PromLens v0.3.0 marked deprecation candidate (UI-06); `playbooks/deploy_docker.yml` reaches `… → alertmanager → grafana → karma → promlens`; 12/13 roles ported (1 remaining: `nfsd`, opt-in for Phase 6); 05-04 closed two regressions — 25 `§` chars swept from karma+promlens non-README files + `_rewrite_uids.py` SUBSTITUTIONS key-form bug (`${VAR}`→`$VAR`) fixed for 3 Type-A dashboards (otel-collector/mimir/tempo) regenerated from upstream + new Gate 9.5 in `roles/grafana/tasks/verify.yml` blocking unresolved panel `"uid": "$..."` refs at deploy time; 12/12 phase-5 must-haves static-verified; 7 items pending live-stack UAT on leviathan (`.planning/phases/05-ui-plane/05-HUMAN-UAT.md`) including the tempo-self-metrics 51-panel upstream-UID classification (warning-vs-blocker pending UAT)*
