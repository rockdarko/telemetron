...
---
*Last updated: 2026-05-19 after Phase 4 plan 04-02 (gap closure) — alertmanager v0.32.1 monolithic role ported with gossip disabled, persistent `telemetron_alertmanager_data` volume, D-61 route knobs, single `null` receiver, one D-63 inhibit rule using corrected `source_matchers:` syntax; Prometheus extended with `alerting: alertmanagers:` block (D-64); 04-02 closed Bug 1 (auto_remove race via `docker_container_exec` rewrite) + Bug 2 (parent-directory bind mounts across 7 roles, eliminating moby/moby#6011 stale-inode class) + meta:flush_handlers belt + Gate 8 doc gate; all 6 Phase 4 UAT tests pass on leviathan; hook router deferred to v2 (ALERT-V2-01..05) via 9-file doc cascade; `playbooks/deploy_docker.yml` reaches `… → fluentbit → alertmanager`; 9/13 roles ported (4 remaining: grafana, karma, promlens, nfsd); Phase 4.1 (2026-05-19) dropped the `vault_` prefix from 8 sensitive variables across 4 roles + renamed `vault.yml.example` -> `secrets.yml.example` + doc cascade (REQUIREMENTS.md OPS-02 / roles/README.md Gate 3 / Phase 1+2 CONTEXT annotations) — see D-90*
